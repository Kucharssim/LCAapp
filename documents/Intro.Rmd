...
---
# Introduction into Latent Class Analysis
...
After we estimated the model, we need to evaulate how it fits the data:
...
<p class="indented"> Where $F_i$ is the observed count for the cell $C_i$ in the multidimensional contingency table and $f_i$ is the implied count in the cell which is computed by the model.
...
Try out the app if you can get retrieve those three groups!
...
1) If the values of log-likelihood per model differ significantly (by amount of units, decimals or hundreths), try to increase the number of replications as that could mean the log-likelihood function has a lot of local maxima. 
